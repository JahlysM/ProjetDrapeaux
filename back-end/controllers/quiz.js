...
export default {
    ...
    Quiz.findMany({
      ...
      .then((data) => {
        const quizWithScore = data.map((quiz) => ({
          ...
          maxScore:
            quiz.quizScore.length > 0 ? quiz.quizScore[0].maxScore : null,
        }));
  ...
  async deleteQuiz(req, res) {
    ...
    await prisma
      .$transaction([deleteQuestionsPromise, deleteQuizPromise])
  ...
  async updateQuiz(req, res) {
    ...
    try {
      ...
      res.status(500).send({
        message:
          error.message ||
          'Une erreur est survenue lors de la mise à jour du quiz ' + id,
      });
    }
  },
  async publishQuiz(req, res) {
    ...
    try {
      ...
      res.status(500).send({
        message:
          error.message ||
          'Une erreur est survenue lors de la publication du quiz ' + id,
      });
    }
  },
};
