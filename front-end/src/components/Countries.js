...
const Countries = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:9000/flags").then((res) => setData(res.data));
  }, []);
  ...
  const handleFlagSelected = () => {
    if (!goodAnswer) {
      ...
      !answer3 &&
      goodAnswer.flag !== selectedFlag.flag &&
      selectedFlag.flag !== answer2.flag
    ) {
      ...
      goodAnswer.flag !== selectedFlag.flag &&
      selectedFlag.flag !== answer2.flag &&
      selectedFlag.flag !== answer3.flag
    ) {
  ...
  const handleCreateQuestion = () => {
    axios
      .post("http://localhost:9000/question/" + quizId, {
        question: question,
  ...
  return (
    <div className="countries">
      ...
      {question &&
        (!goodAnswer ? (
          ...
          <div>
            ...
            <ul>
              <li>
                <img src={imgUrl + goodAnswer.flag} alt={goodAnswer.name} />
              </li>
              <li>
                <img src={imgUrl + answer2.flag} alt={answer2.name} />
              </li>
              <li>
                <img src={imgUrl + answer3.flag} alt={answer3.name} />
              </li>
              <li>
                <img src={imgUrl + answer4.flag} alt={answer4.name} />
              </li>
            </ul>
      ...
      {!question ? (
        <div>
          <h2>écrivez votre question</h2>{" "}
          <input type="text" placeholder="Question" id="questionInput" />{" "}
          <button onClick={handleQuestion}>valider</button>
        </div>
      ) : (
        !answer4 &&
        selectedFlag && (
          <div>
            <p>Vous avez sélectionné le drapeau : {selectedFlag.name}</p>{" "}
            <button onClick={handleFlagSelected}>Valider</button>
          </div>
        )
      )}
      ...
      <ul>
        {data
          .filter((continent) =>
            continent.continent.name.includes(selectedRadio)
          )
          .slice(0, rangeValue)
          ...
          .map((flags) => (
            <li
              className="card"
              key={flags.id}
              onClick={() => handleFlagClick(flags)}
            >
              <img src={imgUrl + flags.flag} alt={flags.name} />{" "}
              {/* ajouter l'événement onClick pour chaque image de drapeau */}
              <div className="infos">
                <h2>{flags.name}</h2>
