...
const GetMyQuizes = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:9000/quiz/myQuizes/" + authorId)
      .then((res) => {
        setData(res.data);
        setIsDeleted(false);
      })
  ...
  const deleteQuiz = (id) => {
    axios
      .delete("http://localhost:9000/quiz/" + id)
  ...
  const updateQuiz = (id, updatedQuiz) => {
    axios
      .put("http://localhost:9000/quiz/" + id, updatedQuiz)
  ...
  const publishQuiz = (id) => {
    axios
      .patch("http://localhost:9000/quiz/" + id)
      .then(navigate("/quiz"))
      .catch((err) => console.log(err));
  };
                ...
                <>
                  ...
                  {quiz.isOnline ? (
                    ...
                    <>
                      ...
                      {quiz.Question && quiz.Question.length > 1 ? (
                        <>
                        ...
                        <p>
                          Ajoutez au moins 2 question pour mettre le quiz en
                          ligne
                        </p>
                      )}
